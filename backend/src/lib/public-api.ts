...
import { ApiConsumerStatus } from "@prisma/client";
import { getCurrentRateWindow } from "../../../src/lib/public-insights";
...
export async function requirePublicApiAccess(request: Request, scope: PublicApiScope, routeKey: string) {
  const rawApiKey = extractApiKey(request);

  if (!rawApiKey) {
    return {
      consumer: null,
      errorResponse: jsonError("API key krävs.", 401),
    };
  }
  ...
  if (!prisma) {
    return {
      consumer: null,
      errorResponse: jsonError("Publika API:t är inte tillgängligt utan databasanslutning.", 503),
    };
  }
  ...
  if (!consumer || consumer.status !== ApiConsumerStatus.ACTIVE) {
    return {
      consumer: null,
      errorResponse: jsonError("Ogiltig eller spärrad API-nyckel.", 403),
    };
  }
  ...
  if (!consumer.scopes.includes(scope)) {
    return {
      consumer,
      errorResponse: jsonError("API-nyckeln saknar rätt scope.", 403),
    };
  }
  ...
  if (existing && existing.requestCount >= consumer.rateLimitPerHour) {
    return {
      consumer,
      errorResponse: jsonError("Rate limit uppnådd för aktuell timme.", 429),
    };
  }
